...
const MISSING_TABLE_PATTERN = /doesn't exist/i;
...
export const ensureSchemaVersion = async (
  ...
  try {
    const rows = await database
      .select({ schemaVersion: mysqlSchema.goodchatMeta.schemaVersion })
      .from(mysqlSchema.goodchatMeta)
      .where(eq(mysqlSchema.goodchatMeta.id, META_ROW_ID));
    const row = rows[0];
